...
model genuinely differs between them (`CODEMAP.md`'s "Data model" section
...
`DESIGN.md`'s existing description of that world's topic instead of
...
1. Add a `crystal(name, type, maxHp, moves, shadeStep?, variantOverride?,
   ...
   - `type` must be an existing `MaterialType` (`TYPE_LOOK`'s keys).
     ...
     `EFFECT_STYLE`, and probably `DESIGN.md`'s type system section) --
   ...
   - `variantOverride` only if this compound's real dimensionality/stacking
     ...
     `'layer'`, a moiré/twisted structure -> `'twisted'` -- see `STYLE.md`).
...
5. If `DESIGN.md`'s per-world table or crystal-database section enumerates
...
6. If `DESIGN.md`'s hybridization section (§5) enumerates specific named
...
1. Extend `BUILT_WORLDS` and add a matching biome entry (`art/biomes.ts`)
   together (`CODEMAP.md`'s "World progression" section) -- also add
...
2. New avatar builder in its own file: `art/<name>.ts`'s
   ...
   `CODEMAP.md`'s "Guardians" list of existing motifs/colors so the new one
...
3. New panel file: `scenes/panels/<name>.ts` exporting
   ...
   `CODEMAP.md`'s "Panel/dialogue UI" section (it enumerates every color in
...
4. Decide the guardian's actual mechanic. If it needs new persisted state,
   ...
   reads/writes it (`CODEMAP.md`'s "Registry-then-persist" pattern) -- and if
...
6. If the guardian teaches passives, `docs/guardians.md`'s relevant table is
   ...
   Update `DESIGN.md`'s guardian roster (§5 or wherever it's documented)
   either way.
