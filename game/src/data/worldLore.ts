...
export const WORLD_LORE: Partial<Record<number, WorldLore>> = {
  1: {
    page1:
      'Long ago, before the corridors had numbers, there was only the Meadow. It was vast. It was symmetric. It was undecided. Its law bound it to nothing: spins up or spins down, either fate as likely as the other. Then a single fluctuation broke the tie -- small enough to fit in a grain of dust. From that instant, the corridor split into two clean, degenerate branches. The Meadow became the first thing in these worlds to ever choose.\n\nThe old story never tells the fine print. A symmetry stays broken forever only in a world large enough to never look back. Something now stalks the Meadow, small enough to make it doubt itself again. The branches that once split so cleanly are bleeding back into one another. An ancient choice is slowly coming undone.',
    page2:
      'Every symmetry that breaks leaves something behind -- a ripple of the order it just carved into being, restless and alive. The old texts give these ripples a common name: quasiparticles, born wherever a choice like the Meadow\'s is made. This meadow is where the first one stirred, and every quasiparticle wandering these worlds still carries a trace of it.\n\nThe Decoherence does not erase that ripple. It strangles it before it can settle into anything with a name -- nothing Qumatex could ever catalog, only noise. Far down this corridor, past where the branches remerge, something ancient waits. It has already finished what this whole meadow is struggling to do. It has been waiting since before you arrived, and it does not intend to lose.',
  },
  2: {
    page1:
      'News of the Meadow travels slowly. But it does travel. By the time it reaches these caves, it has become a legend -- a story told to explain why anything ever chooses at all. The caves themselves never needed to choose. Walk one alcove and you have walked them all. Same shape. Same spacing. Repeated without end. It is a symmetry the Meadow never had: translation, not spin.\n\nThat repetition is not decoration. It is a law -- strong enough that whatever obeys it stops living in any one alcove at all. A state built from this symmetry spreads itself, seamlessly, across the whole tunnel at once, never sitting still in just one place. The caves call this a Bloch state. Until now, nothing has ever caught one standing still.',
    page2:
