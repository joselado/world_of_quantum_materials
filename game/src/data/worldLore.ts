...
export const WORLD_LORE: Partial<Record<number, WorldLore>> = {
  ...
  8: {
    page1:
      "Everyone who camped at the network's edge for a season swears they saw something different move between the trees, and every account contradicts every other -- which, this deep in, nobody finds strange anymore. The lanes end at a treeline standing in fog, and the network could not tell you what is past it. You have seen these trees before, in better light, at the very start; they are dead now. It is not the trees you should be watching. It is what they are standing in. There is a rule every world so far has obeyed: a disturbance in a magnet is one whole spin\'s worth. You can move it, smear it out, watch it travel -- but you cannot have half of one.\n\nWalk into this forest and the path splits -- not into two roads to somewhere, into two halves of the same thing. What entered as one spin\'s worth of disturbance is now two pieces carrying half each, drifting apart, keeping no account of one another. The forest calls the halves spinons, and it does not consider the old rule binding.",
    page2:
...
export const RIVAL_TAUNTS: Partial<Record<number, RivalTaunt>> = {
  ...
  2: {
    part1:
      'A shard-fused golem of graphite-black hexagons unfolds from the shadow between two columns, its faceted skin a thousand distinct crystalline grains stitched edge to edge. "Polycrystalline Graphene Golem. Cross one of my grain boundaries and you\'d think my pattern breaks. It doesn\'t. I rebuild it before you\'ve finished looking."',
    part2:
