...
export const WORLD_FLAVOR: Record<number, string> = {
  ...
  3: 'No single measurement inside a domain tells you its phase: only the Berry curvature, integrated over the whole filled band, and what falls out is always an integer. Where two integers disagree, the gap can only close at the border, and a protected edge channel is what that invariant looks like underfoot.',
  ...
  10: "Nothing here is fixed: the terrain rewrites itself around whatever quantum material you currently are, and its own guardian adapts live rather than defending one static form. Only a variational ansatz, a neural or tensor network trained fast enough to keep up, has any hope of describing either one.",
};
