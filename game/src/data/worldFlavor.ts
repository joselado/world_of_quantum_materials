...
export const WORLD_FLAVOR: Record<number, string> = {
  ...
  4: 'A field this strong bends every trajectory into a closed orbit, and only certain orbits are allowed: each encloses exactly one flux quantum more than the orbit inside it. Electrons fill these Landau levels one flat, massively degenerate rung at a time. The only current that survives runs along the edge.',
  5: "Two electrons that should repel each other bind into a pair, and the paired condensate commits to one phase. A Bogoliubov quasiparticle is part particle and part hole. In a topological superconductor one fermion splits into two halves, each an equal mix of both and so its own antiparticle: pulled to opposite ends, each is a Majorana zero mode, real and unpaired.",
