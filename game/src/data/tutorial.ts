...
export const TUTORIAL_TIPS: Record<TutorialTipId, TutorialPage> = {
  ...
  modes: {
    ...
    unlock: { kind: 'always' },
    body:
      "You pick one of two modes at the Title screen before starting, each keeping its own separate save. Story Mode walks the road: start at World 1, defeat each world's rival to open the next one, and meet guardians as you actually reach them. Superposition Mode is for players who want everything open from the first minute, with no story to work through first: every guardian, transmutation, hybrid, and move is unlocked and every stat maxed out from the moment the save exists, and opponents draw from a flat, tier-scaled challenge instead of climbing world by world. Because the two saves are separate, you can keep a story run going and still go exploring whenever you feel like it.",
  },
  settings: {
    title: 'Settings',
    unlock: { kind: 'always' },
    body:
      "The Lab's Settings station holds eight knobs, in three groups you switch between at the top of the panel. Gameplay: Difficulty scales every stat your opponents get, from an easier B.Sc. tier through the tuned M.Sc. default to a much tougher Ph.D. tier, and it applies starting with your very next battle, so it is meant to be adjusted mid playthrough rather than picked once; Enemy Density controls how many wild encounters the next map you generate will have, and World Size how big that map is: Nano, Meso or Macro, the same world laid out three times as wide and three times as far at the top end. Story: Story Screens and Tutorial Tips decide whether the world lore, the rival taunts and popups like this one stop you as you play. Turn either off and nothing is lost, since everything it would have shown still fills in at the Lab's Story and Tutorial stations, waiting for you there. Presentation: Text Size and Music Style both apply immediately to whatever is already on screen, and Touch Controls puts arrows for walking on screen, on by default when you are playing on a touchscreen.",
  },
  controls: {
