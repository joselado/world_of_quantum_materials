...
// Loops a move's own real battle-effect animation -- the literal
// playAttackEffect BattleScene fires when the move is cast in a fight, sound
// and all, not a separate static icon or a stripped-down copy -- inside a
// guardian panel's detail pane (STYLE.md's "List+detail panels" and
// "Laughlin in the overworld"/"Skłodowska-Curie in the overworld"; Noether's/
// Kondo's/Laughlin's/Skłodowska-Curie's own panels, scenes/panels/noether.ts,
...
// preview escalates into the same multi-trigger, growing-size cascade a real
// leveled cast plays instead of always showing the flat unleveled loop.
//
...
// pushes every Graphics object playAttackEffect creates comfortably above
...
// overlapping play on top of it -- attackEffects.ts's own tweens/Graphics
// have no external cancel hook (every phase tears itself down in its own
// onComplete regardless of anything outside it), so starting a new play
// immediately would leave the old one still drawing until it finishes on its
// own, stacking up visibly with the new one on every rapid row click.
// Instead this just updates the chain's own `current`; the in-flight play's
// own settle callback (afterSettled below) picks up whatever `current` is by
// the time it fires -- the previously-selected move finishes its own cycle,
// then the newly selected one starts, never both drawing at once. A caller
// doesn't need to call stopMoveEffectPreview itself before retargeting a
// chain (e.g. Laughlin's panel rebuilding after a retune), just before
// tearing the chain down for good (Farewell/close) with nothing new to
...
function playNext(key: string, myGen: number) {
  ...
  const { scene, moveClass, from, to, shapeOverride, level } = chain.current;
  ...
  // attackEffectTotalDurationMs for every other shape, since
  // playAttackEffect never calls onComplete for those, see its own doc
  // comment) -- schedules the next cycle after a short pause, re-reading
  // this chain's own `current` fresh rather than closing over these params,
  // so a preview retarget mid-flight takes effect on the very next cycle
  ...
  playAttackEffect(scene, moveClass, from, to, undefined, 1, shapeOverride, isUltimate ? afterSettled : undefined, false, PREVIEW_DEPTH_OFFSET, level ?? 0);

  if (!isUltimate) {
    chain.pendingTimer = scene.time.delayedCall(attackEffectTotalDurationMs(shape, level ?? 0), afterSettled);
  }
}

// Stops one chain (by `key`), or -- called with no key -- every chain at
// once. The currently in-flight single play (if one is mid-animation) still
// finishes and self-destroys on its own (every attackEffects.ts shape tears
// down its own Graphics/tweens in its own onComplete regardless of anything
// outside it); this only stops scheduling another one after it. The no-key
// form is called from a showXPanel rebuild that lands on a state with
// nothing left to preview (Noether's empty-shop branch), and from
// OverworldScene.closeDialogue()/HubScene.closeDialogue() so Farewell/
// leaving the panel for good actually stops every chain rather than leaving
// one scheduled against a scene the player has walked away from. A no-op if
// the given key (or, for the no-key form, every chain) isn't running.
