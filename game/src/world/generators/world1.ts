...
export function generateWorld1Map(gridW: number, gridH: number, start: GridPoint, scale: WorldScale): GeneratedMap {
  ...
  const splitRows = clamp(travel(0.24), 2 * rampRows + scale.tiles(4), scale.tiles(18));
  ...
  const preBands = wanderBands(gridW, start.x, start.y, splitStartY, { width: scale.tiles(WIDE_WIDTH), scale });
  ...
  const postBands = wanderBands(gridW, mergedCenter, splitEndY - 1, goalY, { width: scale.tiles(WIDE_WIDTH), scale });
  ...
  const midIdx = Math.max(0, Math.min(Math.floor(postBands.length / 2), postBands.length - scale.tiles(3)));
