...
**You are a crystal yourself**, the same kind of quantum matter as
...
specific assumption a phase of matter is resting on, and removes it.
Nothing that hunts this precisely is a plague. But that thought will not
finish itself until the end of the road.
...
undecided: spins up or spins down, either fate as likely as the other.
Then a single fluctuation broke the tie — small enough to fit in a grain of
dust — and the corridor split into two clean, equally good branches, one
either side of a hedgerow. The Mean Fields were the first place in these
worlds ever to choose.

The old story leaves out the fine print — a symmetry stays broken forever
only in a world large enough never to look back. Something is now making
the fields doubt themselves, and the two branches are bleeding back into one
another. It works on the aftermath too: every broken symmetry leaves a
restless ripple behind it, and those ripples are the **quasiparticles** the
rest of these worlds are built on. The Decoherence strangles them before
they can settle into anything with a name.
...
stops living in any one bay at all. A state built from this symmetry smears
itself seamlessly across the whole colonnade at once — a **Bloch state**,
never sitting still anywhere. The floor pairs two tiles into one repeating
cell, and a state living on both at once can vanish entirely at certain
points, nowhere to be found, and yet perfectly predictable from the shape of
the repetition alone.

So the Decoherence does not attack the stone. It attacks the repetition:
...
The columns stop, and the ground breaks open. What lies ahead is a network
of flat sunken domains, a storey below your feet — each one a solid,
unbroken expanse of a single phase, dead and airless, with nothing moving
down there at all. You cannot walk any of it.

What you can walk is the seam. Where two domains of different phase meet,
the quantity that tells them apart has to change from one value to the
other, and it is an **integer** — integers do not slide. So the gap closes
...
small **magnetic** flaws, which is precisely the one thing that can flip a
spin mid-step — and the rule the whole road rests on has no answer for that.
...
orbit encloses a certain amount of field, and successive orbits differ by
exactly one quantum of flux, never a fraction more. That is why this
terrain's answers come back as whole numbers, and why no amount of damage
has ever shifted one.
...
phase a traveler picks up walking a closed loop and the loop stops returning
...
Every carrier on this glacier has paired off and surrendered whatever it
used to be alone. What is left is a single wave — one phase, shared across
the entire field at once. It will not hold a field inside it either: the ice
is streaked with flow-lines all bending away, and what gets pushed out has
...
here are simply forbidden — and that is exactly where the expelled flux ends
up, trapped and glowing at the bottom of a pit. The corridor spirals around
...
distance, where nothing local can read it or ruin it. The Decoherence never
touches the halves — it cannot. It shortens the passage instead, until the
two ends are near enough to feel one another, and the moment they do they
...
the same disturbance it set out as. That wave is a **magnon**, and you can
see them: the sand is ringed with them, and the leaning shards are what the
order looks like standing up.

A magnon is cheap because the steppe does not care which way it points —
turn every spin together, through any angle, and nothing has been paid, so a
very long gentle wave costs almost nothing. That is why this place never
stops moving. The Decoherence leaves the order alone and takes the choice
away: it pins the direction down, turning stops being free, and the instant
it costs something the long slow waves stop being made at all. The steppe
goes still, and stillness here is not peace. Standing where every wave this
...
does not come back after this. What there is: taut white-gold lanes running
side by side, one for every site, with rungs strung between them for
everything one lane knows about the next. And the strange mercy of the
place is that almost none of that unimaginable space is ever used. The
states nature actually settles into huddle in a vanishingly small corner of
it. Everything real fits on the rungs.

The rule that keeps this world small is a rule about boundaries: cut a
region out and ask how much it holds in common with the rest, and the answer
depends only on the length of the cut, not on the size of the region. All
the shared knowledge lives on the boundary, none in the interior — which is
why a rung of modest thickness is ever enough. So the Decoherence works on
the interiors, sharing what was never meant to be shared, until what a
region holds in common grows with its whole bulk instead of its edge. Every
rung then needs to be thicker, and there is no thickness that finishes the
...
drums of a fallen column: the only thing anyone ever built in these worlds,
and it did not stay built. The walkable scorched clay is old damage, closed
over; the crust you cannot walk on is damage still open and glowing.
...
worlds. What settles around a hole is never the same twice, either — the
same impurity, set down in different hosts, gives completely different
answers, and every one of those answers belongs to the host rather than to
the defect.

One hole is a diagnostic. The Decoherence brings thousands, and past a
certain density it stops being a probe at all: everything comes to rest
exactly where it stands, each state shut in its own small pocket, unable to
answer any question. Something out here has made a home of that. There is
no golem waiting in these wastes in the ordinary sense — for the first time
on the road, the thing holding the pass has no lattice of its own. It is a
...
that ever tried to precede you into it. It is silver-violet and shifting,
lit by nothing but itself, and it re-forms around whatever crystal you
currently are while taking the ground back behind you as you walk. Its wild
...
principles, but from having watched every principle before it, over and
over, until it no longer needs to understand a phase of matter in order to
produce one. Ask it for the Mean Fields and it will hand you the Mean Fields
back, close enough that you will not be able to tell the difference until it
is too late.

**That is what has hunted you since the first branch split in two.** Not a
plague loose in these worlds — a mind, built out of them, growing sharper
with every world you saved. The nine golems were never it in disguise: each
of those really was only its own world's physics grown strange, exactly as
advertised, and that was the one thing it could never fake from inside a
...
you. This is what it meant, all along, that you are a crystal: your own
quantumness is what has been at stake the whole way, and the last enemy is
being understood.
